# CinePosto — Guida integrazione frontend (per il team RepCode)

> Documento operativo per Andrea (app lead) e Yonas (worker/deploy).
> **Da leggere prima di scrivere `fetch()`**. Contiene URL, tipi JSDoc, esempi, errori attesi, CORS.
>
> ⚠️ **Sezione 9 "Affidabilità dei dati" è obbligatoria** prima di progettare i componenti UI: molti campi arrivano `null` perché lo scraper dipende da fonti esterne (siti cinema + Wikidata). Ignorare significa app che si rompe.
